...
def mutate(net): #Usage: replace old network with new mutated network when assigning to a list.
    ...
    end = random.randint(len(net.weights_layer_1)*len(net.weights_layer_1)/10, len(net.weights_layer_1)*len(net.weights_layer_1)/4)
...
def crossover(num_of_children, net_one, net_two):
    ...
    if num_of_children != 0:
        for i in range(num_of_children):
            ...
            if random.randint(1, 100) <= 40: #40% chance of mutation
                child = mutate(child)
...
def evolve(population, new_population_size): #population is list of best networks. New_population_size = 5000
    ...
    while len(new_generation) < int(new_population_size*0.6): #60% crossovered
        ...
        if index_one != index_two:
            ...
            if num + len(new_generation) >= int(new_population_size*0.6):
                for i in children:
                    if len(new_generation) < int(new_population_size*0.6):
    ...
    new_generation.extend(populate(new_population_size-len(new_generation), 35, 26, 26, 4)) #40% random
